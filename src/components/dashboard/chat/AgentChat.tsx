...
function isResearchReport(content: string): boolean {
  ...
  return h2Count >= 3 && content.length > 600;
}
...
function downloadMarkdown(content: string, docTitle: string): void {
  const blob = new Blob([content], { type: 'text/markdown' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `aivora-research-report-${safeFilename(docTitle)}.md`;
  a.click();
...
async function downloadPDF(
  ...
  doc.text('Aivora Research Report', margin, 34);
  if (docTitle) {
    doc.setFontSize(8);
  ...
  if (citations.length > 0) {
    ...
    for (const citation of citations) {
      ...
      doc.setTextColor(148, 163, 184);
      doc.setFont('helvetica', 'normal');
      const citText = `• ${citation.documentTitle}${citation.pageNumber ? ` (p. ${citation.pageNumber})` : ''}`;
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text('Generated by Aivora', margin, pageH - 20);
  }
                    ...
                    {msg.role === 'assistant' && isResearchReport(msg.content) && (
                      <div className="flex items-center gap-1.5 mt-2 px-1 flex-wrap">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/4 border border-white/8 hover:bg-white/8 hover:border-cyan-400/20 transition-all text-white/40 hover:text-cyan-300 text-[10px]"
                        >
                          {copiedId === msg.id
                        ...
                        {/* Export Markdown */}
                        <button
                          onClick={() => downloadMarkdown(msg.content, msg.citations?.[0]?.documentTitle ?? '')}
                        ...
                        {/* Export PDF */}
                        <button
                          onClick={() => void downloadPDF(msg.content, msg.citations?.[0]?.documentTitle ?? '', msg.citations ?? [])}
                          title="Download as PDF"
                          className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/4 border border-white/8 hover:bg-white/8 hover:border-violet-400/20 transition-all text-white/40 hover:text-violet-300 text-[10px]"
                        >
                        ...
                        {/* Save to Vault */}
                        <button
                          onClick={() => void handleSaveToVault(msg.id, msg.content, msg.citations?.[0]?.documentTitle ?? '')}
                          disabled={savingId === msg.id || savedId === msg.id}
                          title="Save to Knowledge Vault"
                          className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/4 border border-white/8 hover:bg-white/8 hover:border-violet-400/20 transition-all text-white/40 hover:text-violet-300 disabled:opacity-40 text-[10px]"
                        >
                          {savingId === msg.id
                          ...
                          <span>{savedId === msg.id ? 'Saved' : 'Save to Vault'}</span>
                        </button>
                      </div>
                    )}
                  </div>
